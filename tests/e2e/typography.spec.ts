  ...
  const undersized = await page.evaluate(() => {
    ...
    return [...document.body.querySelectorAll<HTMLElement>('*')]
      .flatMap((element) => {
        ...
        if ((text || formText) && parseFloat(getComputedStyle(element).fontSize) < minimum - 0.01)
          failures.push(`${element.tagName}: ${(text || formText).slice(0, 70)}`);
        for (const pseudo of ['::before', '::after']) {
          const style = getComputedStyle(element, pseudo);
          if (
            !['none', 'normal', '""'].includes(style.content) &&
            parseFloat(style.fontSize) < minimum - 0.01
          )
            failures.push(`${element.tagName}${pseudo}: ${style.content}`);
        }
        return failures;
      })
      .slice(0, 12);
