...
for (const route of ['overview', 'video', 'architecture', 'tools', 'cost', 'roadmap', 'sources', 'sources?tab=verified']) {
...
test('klavye araması, Escape ve odak geri dönüşü', async ({ page }) => {
  await page.goto('');
  const trigger = page.getByRole('button', { name: 'Araştırmada ara' }).filter({ visible: true }).first();
  await trigger.focus();
