...
İlk CI çalışması, mimari sayfasındaki kayan kod bloklarının klavye odağı almadığını yakaladı: 67 test geçti, iki axe kontrolü başarısız oldu (`evidence/v4/red-ci-accessibility.txt`). Yerel regresyon testi de eksik odağı doğruladı (`red-code-keyboard.txt`). Kod bloklarına erişilebilir ad, bölge rolü ve klavye odağı eklendi. WebKit mobil profilinde yerel sağ ok kaydırması çalışmadığından değiştirme tuşlarına dokunmayan sağ/sol ok kaydırması eklendi. Test, gerçek klavye tuşuyla `scrollLeft` değişimini doğrular. Son yerel sonuç **72 tarayıcı testi**: 66 Chromium, 6 WebKit (`green-browser-final.txt`).
